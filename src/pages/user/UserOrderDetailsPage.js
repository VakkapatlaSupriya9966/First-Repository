...
const loadPayPalScript = (cartSubtotal, cartItems, orderId, updateStateAfterOrder) => {
    loadScript({"client-id":
        "ARMlGN9A3zp3Kuqs7kTrzoqXtntCer2YbjmDcrxwVxItcwP32eNsaJwtH9xC1RYEkQIck9vymL-p68bG"})
        .then(paypal => {
            paypal
    ...
    return {
        ...
        onApprove: function(data, actions) {
            return actions.order.capture().then(function (orderData) {
                ...
                if (transaction.status === "COMPLETED" && Number(transaction.
                    amount.value) === Number(cartSubtotal)) {
                       updateOrder(orderId)
                       .then(data => {
                            if (data.isPaid) {
